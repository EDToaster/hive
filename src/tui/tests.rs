...
fn make_agent_with_status(id: &str, status: AgentStatus) -> Agent {
    Agent {
        id: id.into(),
        role: AgentRole::Worker,
        status,
...
fn make_task_with_status(id: &str, status: TaskStatus) -> Task {
    make_task(id, "title", None, status)
}

#[test]
fn detect_new_agent_generates_spawn_notification() {
    let agents = vec![make_agent_with_status("agent-1", AgentStatus::Running)];
    let tasks: Vec<Task> = vec![];
    let prev_agents: HashMap<String, AgentStatus> = HashMap::new();
    let prev_tasks: HashMap<String, TaskStatus> = HashMap::new();

    let (notifs, ring_bell) = detect_state_changes(&agents, &tasks, &prev_agents, &prev_tasks);
    assert_eq!(notifs.len(), 1);
    assert!(notifs[0].message.contains("spawned"));
    assert_eq!(notifs[0].severity, NotificationSeverity::Info);
    assert!(!ring_bell);
}

#[test]
fn detect_agent_failed_generates_error_and_bell() {
    let agents = vec![make_agent_with_status("agent-1", AgentStatus::Failed)];
    let tasks: Vec<Task> = vec![];
    let prev_agents: HashMap<String, AgentStatus> =
        [("agent-1".to_string(), AgentStatus::Running)].into();
    let prev_tasks: HashMap<String, TaskStatus> = HashMap::new();
...
#[test]
fn detect_agent_stalled_generates_warning_and_bell() {
    let agents = vec![make_agent_with_status("agent-1", AgentStatus::Stalled)];
    let tasks: Vec<Task> = vec![];
    let prev_agents: HashMap<String, AgentStatus> =
        [("agent-1".to_string(), AgentStatus::Running)].into();
    let prev_tasks: HashMap<String, TaskStatus> = HashMap::new();
...
#[test]
fn detect_agent_done_generates_info_no_bell() {
    let agents = vec![make_agent_with_status("agent-1", AgentStatus::Done)];
    let tasks: Vec<Task> = vec![];
    let prev_agents: HashMap<String, AgentStatus> =
        [("agent-1".to_string(), AgentStatus::Running)].into();
    let prev_tasks: HashMap<String, TaskStatus> = HashMap::new();
...
#[test]
fn detect_task_merged_generates_info() {
    let agents: Vec<Agent> = vec![];
    // Two tasks, one still active — so "all tasks complete" won't trigger
    let tasks = vec![
        make_task_with_status("task-1", TaskStatus::Merged),
        make_task_with_status("task-2", TaskStatus::Active),
    ];
    let prev_agents: HashMap<String, AgentStatus> = HashMap::new();
    let prev_tasks: HashMap<String, TaskStatus> = [
        ("task-1".to_string(), TaskStatus::Active),
        ("task-2".to_string(), TaskStatus::Active),
    ]
    .into();
    ...
    assert_eq!(notifs.len(), 1);
    assert_eq!(notifs[0].severity, NotificationSeverity::Info);
    assert!(notifs[0].message.contains("merged"));
    assert!(!ring_bell);
}

#[test]
fn detect_all_tasks_complete_generates_notification() {
    let agents: Vec<Agent> = vec![];
    let tasks = vec![
        make_task_with_status("task-1", TaskStatus::Merged),
        make_task_with_status("task-2", TaskStatus::Merged),
    ];
    let prev_agents: HashMap<String, AgentStatus> = HashMap::new();
    // Previous had one active task → not all done before
    let prev_tasks: HashMap<String, TaskStatus> = [
        ("task-1".to_string(), TaskStatus::Active),
        ("task-2".to_string(), TaskStatus::Active),
    ]
    ...
    let (notifs, _) = detect_state_changes(&agents, &tasks, &prev_agents, &prev_tasks);
    // Should include task merged notifications + all complete
    let all_complete = notifs.iter().any(|n| n.message.contains("All tasks"));
    assert!(all_complete);
}

#[test]
fn no_change_generates_no_notifications() {
    let agents = vec![make_agent_with_status("agent-1", AgentStatus::Running)];
    let tasks = vec![make_task_with_status("task-1", TaskStatus::Active)];
    let prev_agents: HashMap<String, AgentStatus> =
        [("agent-1".to_string(), AgentStatus::Running)].into();
    let prev_tasks: HashMap<String, TaskStatus> =
        [("task-1".to_string(), TaskStatus::Active)].into();
